...
export interface LeverageCalculatorProps {
...
function FreeLeverageCalculatorInner({ onLogTrade }: LeverageCalculatorProps) {
  ...
  }, [selectedAsset, direction, stopLoss, allowedMarginLossPercent, tranches])
  ...
  const handleTriggerLogTrade = () => {
    ...
    const data: TradeData = {
      ...
      direction,
      entryPrice: avgEntryPrice > 0 ? Number(avgEntryPrice.toFixed(2)) : 0,
      leverage: calculatedLeverage > 0 ? calculatedLeverage : 1,
      margin: totalMargin > 0 ? Number(totalMargin.toFixed(2)) : 0,
    ...
    <div className="w-full max-w-4xl mx-auto space-y-8 text-slate-100 font-sans pb-20">
      ...
      <div className="text-center space-y-4 pt-4 pb-2 max-w-2xl mx-auto">
        <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-brand-muted border border-brand-border text-brand text-xs font-mono font-semibold tracking-wide">
          ...
          Pro Suite • Precision Leverage Engine
        </div>
        ...
        <p className="text-xs sm:text-sm text-slate-400 leading-relaxed max-w-lg mx-auto">
          Compute optimal liquidation buffers, blended DCA entries across multiple tranches, and directly sync your risk parameters into your journal.
        </p>

        <div className="flex flex-wrap items-center justify-center gap-5 text-xs text-slate-400 font-mono pt-2">
          <span className="flex items-center gap-1.5">
            <ShieldCheck className="w-4 h-4 text-emerald-400" /> Dynamic Fee Inclusion
          </span>
          ...
          <span className="text-slate-700">•</span>
          <span className="flex items-center gap-1.5">
            <ShieldCheck className="w-4 h-4 text-emerald-400" /> Direct Journal Sync
          </span>
        </div>
      </div>
...
export default function LeverageCalculator(props: LeverageCalculatorProps) {
  return (
    <Suspense fallback={<div className="min-h-screen bg-[#0a0d14] flex items-center justify-center text-slate-400 font-mono text-xs">Loading calculator...</div>}>
      <FreeLeverageCalculatorInner {...props} />
    </Suspense>
  )
}